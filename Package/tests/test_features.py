...
from bikeshare_model.processing.features import year_month, WeekdayImputer, WeathersitImputer, Mapper, WeekdayOneHotEncoder
...
def test_weekday(sample_data_pull):
    #Given
    mod = WeekdayImputer(variable=config.modelConfig.weekday_var, date_var=config.modelConfig.dteday_var)
    assert sample_data_pull[0][config.modelConfig.weekday_var].isna().sum() != 0
    
    #When
    sample_data_pull[0][config.modelConfig.dteday_var] = pd.to_datetime(sample_data_pull[0][config.modelConfig.dteday_var],format='%Y-%m-%d')
    out= mod.fit(sample_data_pull[0]).transform(sample_data_pull[0])
